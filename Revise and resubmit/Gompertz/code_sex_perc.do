...
import excel "C:\Users\baurin\Documents\UCL Doctorat\Formation\Thèse\Papier 2 définitif\Code\Data simulation\By income percentile and sex\Distribution - Gompertz.xlsx", sheet("Sheet1") cellrange(A1:OJ81) firstrow clear
...
foreach j in F M {
forvalues i = 1(1)100 {
gen P`m'_WeightDeviation`j'`i' = (abs(Age_ind -`m'))^B * C`j'`i' * `j'`i'
}
}
}
...
dis as text "With only one retirement age, an error aversion of " as result `=B' as text ", the social planner would set the retirement age at "   as result Retirement_age[1] as text "."
...
dis as text "With one retirement age by sex, an error aversion of " as result `=B'   as text ", the social planner would set the retirement age at "   as result Retirement_age_`j'[1] as text "."
}
restore 
}
...
foreach j in F M {
forvalues i = 1(1)100 {
gen P`m'_WeightDeviation`i'_`j' = (abs(Age_ind -`m'))^B * C`j'`i'*`j'`i'
}
}
}
...
dis as text "With one retirement age by percentile, an error aversion of " as result `=B' as text   ", the social planner would set the retirement age at "   as result Retirement_age_`i'[1] as text "."
}
restore
}
...
dis as text "With one retirement age by sex & percentile, an error aversion of " as result `=B'  as text ", the social planner would set the retirement age at "   as result Retirement_age_`i'_`j'[1] as text "."
