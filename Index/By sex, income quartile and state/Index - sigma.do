...
import excel "C:\Users\baurin\Documents\UCL Doctorat\Formation\Thèse\Papier 2 définitif\Code\Data simulation\By state, income quartile and sex\Distribution - Chetty technique.xlsx", sheet("Sheet1") cellrange(A1:AEJ81) firstrow
...
foreach j in F M {
foreach k of numlist 1 2 4/6 8/13 15/42 44/51 53/56 { 
replace P`m'_WeightDeviation`i'`j'`k' = s * P`m'_WeightDeviation`i'`j'`k' if Age > `m'
}
}
}
}
...
foreach j in F M {
foreach k of numlist 1 2 4/6 8/13 15/42 44/51 53/56 { 
replace P`m'_WeightDeviation`k'_`j'`i' = s * P`m'_WeightDeviation`k'_`j'`i' if Age > `m' 
}
}
}
}
...
dis as text "With one retirement age by state, an error aversion of " as result `=B' as text ", a ponderation of " as result `=s' as text " and a proportion of life spent working " as result `=alpha' as text ", the social planner would set the retirement age at " as result `=alpha' as text " * " as result Retirement_age_`j'[1] as text "."
}
restore 
}
...
dis as text "With one retirement age by state and by sex, an error aversion of " as result `=B' as text ", a ponderation of " as result `=s'  as text " and a proportion of life spent working " as result `=alpha' as text ", the social planner would set the retirement age at " as result `=alpha' as text " * " as result Retirement_age_`j'_`s'[1] as text "."
}
}
restore 
}
...
foreach j in F M {
foreach k of numlist 1 2 4/6 8/13 15/42 44/51 53/56 { 
replace P`m'_WeightDeviation`k'_`j'`i'_ = s*P`m'_WeightDeviation`k'_`j'`i'_  if Age > `m'
}
}
}
}
...
dis as text "With one retirement age by state, by quartile and by sex, an error aversion of " as result `=B' as text ", a ponderation of " as result `=s'  as text " and a proportion of life spent working " as result `=alpha' as text ", the social planner would set the retirement age at " as result `=alpha' as text " * " as result Retirement_age_`j'_`s'_`q'[1] as text "."
}
}
}
restore 
